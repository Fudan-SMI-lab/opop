...
def cmd_doctor(args) -> int:
    ...
    # GPU driver (replaces the old `wsl.exe -l -q` distro probe: on native Linux the
    # orchestrator and the worker are the same OS, so there is no distro to look for.
    # The real precondition is a working driver, checked here; torch/triton/CUDA are
    # checked below by the env-probe job, which actually launches a kernel.)
    try:
    ...
    # opencode CLI
    try:
        from kernel_optimizer.agents.runtime import resolve_opencode
