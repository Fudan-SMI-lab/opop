...
**状态**:设计稿,2026-09-09(经调研修订)。v3 分支从 v2 的 `8772831` 继承全部代码,`D:\Pyhon_projects\opop\v3`(git worktree,同仓库分支 v3)。v2 分支继续跑完当前两个实验,box 1 完成后转为 v3 的实验平台。

本文档定**整体思路、项目逻辑与设计约束**。实施细节见 `v3-implementation-plan.md`。调研出处与借鉴清单见 `v3-prior-art-and-what-to-borrow.md`。
...
### 修订 5:向量合成一个数的规则已有确切形式(ECM),重叠程度是可测标量(α)——但对我们仍是探索项

初稿在 §二缺口 2 写"我们目前隐含用 max(取最紧的那维),应把假设写明"。**ECM 给了确切形式**(Hofmann/Eitzinger/Fey,arXiv:1509.03118;Stengel 等 ICS'15):

```
T_ECM = max( T_nOL + T_data , T_OL )
```

**既不相加也不取 max,而是结构化的部分重叠**:核内工作按"能否与数据传输重叠"分两类,不可重叠的那部分与传输时间**相加**,其余与该和取 **max**。**两条重叠假设是模型的可证伪内容**,ECM 自己按层级报误差列来检验它们。

**ERM 把"重叠多少"变成一个标量**:`α = T_overlap / min(T_x,T_y) ∈ [0,1]`,原文语义 "If α = 0 there is no overlap and the total execution time is the sum … if α = 1, the overlap is maximal"。全局界 `max_x(T_x) ≤ T ≤ Σ_x T_x`。

**为什么这对我们重要**:α 正是改写 agent 一直在猜的那个数 —— **为省流量而多花的算力,是免费的(α≈1)还是一比一付账的(α≈0)。** fp16/tf32、重算、融合三类改写全押在这个未知量上。

**为什么它仍是探索项而不是确定项**:α 的定义需要"该维度单独的分量时间"。ECM/ERM 在 CPU 上从 OSACA 静态汇编分析与分层传输模型取这些分量;**我们在 GPU 上取哪两个分量、怎么取,还没有可执行方案** —— 这与 §二缺口 1 是同一个瓶颈。子代理称"两点墙钟实验即可",**我不采信**:它没有说这两点具体是什么。写进设计,不排进关键路径。

**但这一族的可用性事实是决定性的,值得单独记**:Kerncraft(从循环源码 + 机器描述自动建 Roofline/ECM,含预测最优 blocking 因子的 layer-condition 分析)与 OSACA(从汇编静态算 `T_nOL`/`T_OL` 切分)**整族 counter-free**。**这是唯一一个能在我们的租用容器里原样跑的先验工作族。** 我们已在读 cubin(Tier 1),技术上这条路是通的。
...
**性能模型那一路又给了三条更强的、且都 counter-free 的**:

- **向量的闭合性必须靠构造,不靠期望(TMA 的两条结构规则)。** Yasin 的 top-down 之所以百分比真能加到 100%,是因为 (1) **分母是守恒的可数资源**(每周期的每个 issue slot 被决策树分入且仅分入一类,加和为 1 是"划分"的推论而非事后归一化),(2) **有且仅有一个残差类**(`Backend_Bound ≡ 1 − 其余`)。**照抄这两条**:把 `total_ms` 划分为具名维度,并把 `unexplained_ms` **定义为**残差。这也正好回答"某一维不可测时向量还能不能加总"——能,只要不可测的份额被显式归入残差。
  **输出形状借它的 "Bottleneck View"(扁平约 12 项、具名、加和 100%),不借那棵 6 层树** —— 树的存在理由是 Intel 有上千 PMU 事件需要下钻,而我们零 counter 且消费者是要据此行动的 agent。**其中最该抄的一项:把内存拆成"带宽压力"与"延迟暴露"两个维度**(TMA 是 `Cache_Memory_Bandwidth` vs `Cache_Memory_Latency`,ERM 是 `U_issue`/`U_lat`)——这恰好区分"减字节"与"加 MLP/occupancy"两种完全不同的改写策略,而我们目前不做这个切分。
  **一个诚实的差别要一起记**:GPU 侧最接近的实现 DrGPU(ICPE 2023)靠"对各层平均停顿之和归一化"闭合,**既非守恒划分也无残差项** —— 它保证百分比加到 100%,但不保证它们**意味着**损失的 100%。我们若采这条更弱的路,必须在输出里把这个警告一起说出来。
...
- **阶段 1(最高价值,最便宜)**:把编译期可知的不可行**从空间里声明掉**,而不是抽出再拒。判据:`infeasible_shared_memory` 的 record 数应大幅下降,且早期搜索质量改善(依据 §1.3 的两个外部实测)。
- **阶段 2(原料齐备)**:判决从标签改为**向量 + 消化层**。向量按 Halide 的形状加密度(分层字节/工作集、unique vs total)、加上四个已在采集但无人读取的 SASS 计数器、加峰值显存;**按 TMA 的两条结构规则闭合(守恒分母 + 显式残差项 `unexplained_ms`)**;**把内存拆成带宽压力与延迟暴露两维**(TMA 与 ERM 独立到达的同一条缝);判决输出**哪个维度绑定 + 其余维度余量**(MAESTRO 形),三区分类(cuPilot 形)。**给 agent 的必须是消化后的诊断,不是数值**(§零修订 1)。
...
- **阶段 3**:每维**下界**作为"还剩多少空间"的锚;`CUDABench` 的 `Performance-Score = Achieved/Attainable` 作为头条上报量(可无 counter 计算、跨卡稳定)。**同时按 §1.2 的 ceiling-order 教训:屋顶的可达性必须按任务从实测推导,不得作为常量写进 prompt。**
- **阶段 4(转化效率)**:Roller 的 `S=ΔQ/ΔF` 作**静态估计器**用于 trial 排序;**已实现**的 `Δlatency / Δresource` 作报告与 prompt 信号。三条硬约束:不作 TPE 目标、噪声底以下报 unknown、不作筛选剪枝(v2 实测反例:L2:37 上**最慢**的种子所属族改善最多 −30.5%,按效率剪枝会杀掉赢家)。**第四条(新增,来自 §四 第 6 点)**:每维报**价格区间**而非点值,区间重叠时拒绝行动 —— 这是对偶退化的标准处理,也正好是我们 33 个近平局的处方。
- **阶段 5**:候选真实流量测量(**探索项**,缺口 1)。
- **阶段 5b(与 5 同一瓶颈,探索项)**:**重叠系数 α**(§零修订 5)。ECM 的 `max(T_nOL+T_data, T_OL)` 给了合成规则,ERM 的 α 给了可测标量,但"在 GPU 上取哪两个分量时间"没有可执行方案。**若 5 通了,5b 才有意义。**
- **阶段 6**:通信维度,为多卡/异构预留。

**一条排序自检(借 Gables 的算例)**:Gables 的四步算例是这套思想最干净的公开演示 —— 40 → **1.3** → **2** → **160** Gops/s,其中三步都是"把某个资源朝局部正确的方向动然后变差",第三步甚至是花 3 倍带宽只换 1.5 倍、然后把带宽还回去改提复用。**我们的框架应当能复现这四个配置的排序**;做不到就说明合成规则错了。这个检查不需要 GPU。
...
6. **把 LP 对偶/影子价格用于代码或 kernel 优化的瓶颈归因** —— 三种检索方式零命中,且该次检索有工作的正对照(同一方法正确找到了 ERM/DrGPU/Gables/TMA)。**借词汇与两条定理,不建 LP**:(a) **互补松弛作可证伪自检** —— 若模型判定某维松弛,则在该维花资源应买到**零**改善,这是一次廉价消融就能做的证伪;(b) **对偶之比 = 替代率** —— 把转化效率定义为 `Δlatency/Δresource_j`,两维交换率取二者之比,使它成为有量纲、可比较的量。**并预期退化**:33 个近平局 + 每 trial std 占均值 16% 是教科书式的对偶退化(多资源同时接近绑定,各自价格不可定),文献的答案提示我们应**每维报价格区间而非点值,区间重叠时拒绝行动**。
