...
- ~~**向量的闭合性靠 TMA 的守恒分母 + 显式残差**~~ —— **已撤回(用户否决"合成一个数"这个方向)**。把 `total_ms` 划分成具名分量并要求加和守恒,**本质就是合成**:它假定分量可比、可加、**数目固定**,而这与"维度数随设备变、且每次结构改动后占用率都变"直接冲突。详见 `v3-revision-no-scalarization-and-retire-risk.md`。
  ...
  **替代设计:按维度独立,维度集合运行期发现。** 每个维度是一条独立记录:
...
7. **拒绝合成:维持多份并列的按维度判决,且维度集合本身是运行期发现的。**(2026-09-10 新增,来自用户对设计方向的否决)

   调研 §3.9 的结论是:**现存所有多资源模型最终都收敛到一张 2D 图或一个标量界** —— ERM 用 α 合成、ECM 用重叠假设合成、Gables 用 `max` 合成、Nsight SOL 在维度间取 max、CUDABench 的 `Performance-Score` 把两维压成一个标量。**"拒绝合成、维持多份并列判决、并把维度集合当作随设备与结构变化的运行期产物"在约 120 篇调研里没有对应物。**
