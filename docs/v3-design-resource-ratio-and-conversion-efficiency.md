...
> **⚠ 部分已被取代（2026-09-10 第三版)**：用户提出三条纠正后，本文档中关于 **S2c 局部资源地图**、
> **S4 的静态估计器 `S=ΔQ/ΔF`**、以及**「资源间转化效率 = 资源对的属性」**的部分已作废。
> **当前有效版本是 `docs/v3-revised-design-and-implementation.md`**。本文档的**论证、实测数据与调研出处**仍然有效，
> 且其中的三重否证（0/96 无闭式公式、0/10 不可分离、符号不可靠）在新版里改为支持「不给 agent 任何变化率」。
