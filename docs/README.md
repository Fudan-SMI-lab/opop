...
| **`result-a1-evidence-base-had-two-defects.md`** | **A-1 的证据基自己有两个缺陷**:(1) 语料漏了**唯一 DRAM 真绑定的 L3:48** —— 而它正是催生 A-1 的案例(补进后 n=61,0.85 阈值下首次出现 `dram+registers` 组合);(2) `argmax |Δ压强|` 这个标签被**量纲**污染 —— shared 的中位摆幅是 registers 的 **13.5 倍**,于是"最佳对照"变成"永远挑 shared"(61.5%),它什么都没发现。**主结论不变**(R1 最紧的墙优先 7.7%,全表最差),但"多数改写没缓解绑定维度"下调为"**约一半**";新纪律 **A-3b:禁止跨维度 argmax** |
...
| `a1_recheck_with_l348.py` | **A-1 的结论在补进 L3:48、换成编译期维度后还成立吗** | **成立但数字要改**:多维绑定 72.1%(n=61);"动得最多的那维是绑定的" 26.1% → **47.8%**;缓解绑定 10 vs 什么都没缓解 11(对半) |
