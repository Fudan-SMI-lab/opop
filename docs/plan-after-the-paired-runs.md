...
**2026-09-12 新增,同一处的更便宜的一半 —— 见 D9**:`59d5a71` 建了 `prescreen_timeout_s`
并写了 30 行论证,却漏掉同文件 88 行外的 **`compile_screen`**,后者仍借用 `build_timeout_s`(1200 s)。
实测三箱 1108 次单配置筛查 **p99 ≤ 105 s(截止线是它的 11 倍)**,而 1200 s 筛 + 1800 s 评正好
=== box1 那四个 **50.1 分钟**空档。单配置筛查占 run 跨度 **34.1% / 26.6% / 23.4%**,且**不预热缓存**
(唯一跑完的慢筛 991.0 s,紧随的 eval 又 978.8 s)。**把 `compile_screen` 也走 `prescreen_timeout_s`
是一行改动**,应与 B2 一起做(同为筛查定价),但它不需要 B2 的设计工作。
...
3. E1  S2d(c) 配对实验                            ⚠️  已跑完但**对照臂 0 轮改写** → 需重跑
   ↕ 并行: E3 L3:48 A800 干净复现                 🟢 已跑完 (待落盘复核)
4. D9 + D8 修复  ← **新增,插在这里**              (D9 方案已备; D8 需先测 PTX 规模分布)
5. E1-r  重跑配对实验(两臂都带 D9+D8 修复)
6. 分析 E1-r/E3 → 决定 E2 的臂数与预算
7. E2  三臂实验(label / vector / vector+numbers)  (12 h × 3 或 18 h × 3)
8. B1 + B2  预算效率修复                          (实验之间做, 不在实验中途)
9. B3  correctness_mismatch 归因                  (独立课题, 最后)
```

**为什么 D9+D8 必须插在 E1 重跑之前**(2026-09-12 落盘结论):E1 对照臂在
**02:35:49 → 08:41:46 有 6.10h 零 agent 调用**,窗口内只有一个候选的 40 个 trial
(9.15 min/trial)= **预算的 50.8%** ⇒ 该臂 12h 跑完**从未进入 loop C**。
自变量 `reserve_round_for_reconciled` 作用于**一轮改写**,而对照臂改写数为 0 ⇒
**不存在对自变量作用对象的对照观测**。不先修这两个缺陷,重跑会重现同一结果。
两者互补:**D9 只回收约 2.1h**,而那 6.10h 主要是**答出来的**作业 ⇒ 属 D8 不属 D9。

**关键约束**:B1/B2 都改变 TPE 访问的点或采样,**必须在两次实验之间**做,不能在一对实验的中途 —— 否则
两臂不可比。A1/A2 同理,但它们在 E1 之前,所以 E1 的两臂会同时带上它们,是对等的。
**D9/D8 的修复同样改变可比性**,所以它们必须在 E1 重跑**之前**一次做完,不能分两次。
