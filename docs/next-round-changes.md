...
| **2** | `n_spills` 软墙检出 | 建议做,**未批准** | 需**新判据函数**(不是给 `find_walls` 加参数) | 0(探针已采集该字段) | ❌ 不需要 | 软墙不产生被拒参数组,现有输入结构读不到 |
| **2'** | `occupancy` 软墙检出 | 建议做,**未批准** | 同上 | 0 | ❌ 不需要 | 极性**反**(低才绑定);**嵌套字段**,平读会把"已测"读成"未测" |
...
| **3.2** | **S7** 斜率引导采样 | **待批准**,高风险 | `tpe.py` 注入点(照 S1b 接线) | **一对 12h** | ✅ **需要** | 破坏 run 间可比性;"斜率是好先验"**未证实**(ρ ≤0.24) |
