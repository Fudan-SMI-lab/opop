...
## §4.8 【用户指出,实盘证实】斜率与墙判据都建立在**未条件化的边际统计**上,这是零投递的上游根因

**用户的两点判断(2026-09-14),经代码与 step-4 全开臂实盘数据核实,均成立。**

### (a) 斜率是"每值边际中位",搭档不固定 ⇒ 曲线一半在测采样史,不在测 knob

代码链条:`stats.py:79-84` 把**所有**完成 trial 按该 knob 取值分桶、每桶取中位
(`latency_by_value`);`wall_attribution.find_walls` 与 `soft_wall.find_soft_walls` 的
尾部斜率、`monotone` 判定全部读这条边际曲线,**没有任何搭档条件化**。
桶的成分由 TPE 的采样史决定 —— 它对喜欢的值配好搭档、对放弃的值只留坏搭档,
所以边际曲线混合了 knob 效应与采样器偏好,是 [[pooling-register-classes-inverts-correlations]]
(Simpson 悖论)在斜率估计器里的重演。

**实盘(s4-c2on,cand-94aab0fc / `G_BLOCK_M`,即 seq622 被丢弃的墙,tail_gain +72.8% 却
`monotone=False`)**:

| G_BLOCK_M | n | 边际中位 | 该值上最好 |
|---|---|---|---|
| 32 | 7 | 6.61 | 4.06 |
| 64 | **2** | 19.58 | 11.45 |
| 128 | 17 | **30.67** | **3.28** |
...
边际中位是 6.6→19.6→30.7→5.3 的锯齿(⇒ 非单调 ⇒ 墙被丢);每值最好是 4.06/11.45/3.28/3.19。
128 桶的中位与自己的最好差 **9 倍**(桶里塞满 `ieee` 等坏搭档,最差 103ms)。
同臂 `ATT_STAGES` 尾部三值的 n = 44/7/**1** —— 单调性建立在 n=1 的桶上。
**该臂 5/5 道成形的墙全死于 `monotone=False`,而锯齿主要是桶成分噪声。**
这也给了 S7"斜率大小预测被反向否证"([[the-slope-magnitude-prediction-was-disconfirmed-shallow-points-lost-least]])
一个机制解释:`tail_gain_pct` 本身主要是噪声,自然不预测投递点胜负。

### (b) 墙判据"换搭档跑通即抹墙"在乘积型约束下结构性失效

`find_walls` 要求被拒值**在边际已测范围之外**;共享内存 ≈ tile 各维乘积,任何单值都存在
可行搭档 ⇒ 采样越充分墙越少([[a-wall-can-be-erased-by-a-later-successful-trial]] 已实测全过程)。
全开臂 8 次归因事件里 4 次 `walls_found=0`:35 个拒绝**每个被拒值都在边际范围之内**。
拒绝全部真实(S7 复核 39/39 超限 1.13–2.26x)——**这个判据把"墙"定义成了采样不完整性的残差,
TPE 干得越好它越接近零**,永远赶不上每 10 trial 重算一次的消费节奏。

### 关键细化:行动端是条件化的、对的;测量端是边际的、错的

消融探针(θ\* 固定其余 knob 动一个)与投递点构造(`_toward_wall`:θ\* 搭档 + 走一步)
都已条件化 —— 但决定它们**是否被触发**的入口闸门(`find_walls` 范围判据 +
`select_for_probing` 单调过滤)跑在边际曲线上。
**⇒ 管道用被污染的边际统计守门,把条件化的正确动作挡在门外。**
0 投递的两个直接死因(值在范围内 ⇒ 无墙;锯齿 ⇒ 非单调)都是测量端产物。

### 为什么当初退到边际中位,以及为什么仍不成立

严格"只差一个 knob"的配对在记录里只有 19/2326(1%)—— 条件化曲线从观测数据里画不出来。
但结论应当是**不从观测数据读斜率**(像资源维那样花探针预算实测:compile-only 7ms/个 +
每批 11s 固定成本已在 §4.5 计价),而不是退回被污染的估计器。

### 修法(优先级)与红线

1. **墙判据条件化**:"该值在 θ\* 的搭档下不可行"(消融探针本就在回答这个问题;
   §4.5 的多 knob 回退是其推广)⇒ 墙不再被无关搭档的成功抹掉。
2. **斜率改为 θ\* 处局部实测**(沿 knob 轴从 θ\* 出发的少量定点 trial),
   或至少给发射端每桶 n 设披露门槛(win.py 读端已有 `--min-n`,发射端没有)。
3. **正对照必须带**:改判据后仍须存在被判"非墙"的拒绝,否则"墙"退化为"任何一次拒绝"
   (与 §4.5 的正对照要求同一条)。不缩空间、不加硬编码。

### 与已有条目的关系(三个缺口,层层上游)

- §4.5:墙进来了,**归因探不到**(单 knob 射程);
- 本节 (b):墙**根本进不来**(边际范围判据);
- 本节 (a):进来了也**被假锯齿杀掉**(边际斜率过滤)。
- [[a-shared-memory-refusal-can-arrive-labelled-runtime-error]]:约 2% 的拒绝连输入都进不了。

修 §4.5 而不修本节,处理臂类 run 仍是 0 投递(§4.5 的"两臂成因不对称"表已说明);
修本节 (b) 而不修 (a),墙进来了仍会被单调过滤杀掉。**三者要一起设计。**
