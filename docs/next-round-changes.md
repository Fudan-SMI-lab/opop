...
### 已离线量化:38 个改写,17 个可判定,**4 个(24%)是空操作**

**零机时**,读 box4 上 6 个 run 的 `events.jsonl`
(`inherited-from-box2` ×2、`arm2`、`arm3`、S7 两臂),
工具 `scripts/probes/rewrite_source_vs_knob.py`(8 项自检,含 identity/moved 双向区分):

| | |
|---|---|
| 改写总数 | 38 |
| **可判定(存在匹配点)** | **17** ← 唯一诚实的分母 |
| 不可判定 | 21(空间仍开 5、取值从未被采样 16、无同族 0、knob 集不交 0) |
| 源码改动**动了** ≥1 个已测维度 | **13 / 17(76%)** |
| 源码改动在全部维度上**恒等** | **4 / 17(24%)** |

动的时候动哪些维度:`n_regs` 10、`shared_bytes` 9、`n_spills` 6、`occupancy` 4、
`num_warps` 3、`occ_limiter` 1。

**内部一致性检验(不是巧合)**:4 个 identity 案例的固定 knob 延迟差是
**+0.12% / −0.12% / +0.28% / +0.02%,全部落在 ±0.3% 内**;
13 个 moved 案例跨 **−26.07% … +26.81%**。
零资源变化 ⇄ 零延迟变化,两个独立读数互相印证 ⇒ 这 4 个确实什么都没做,
不是"匹配逻辑漏了什么"。

**选择偏差已排除(这是我担心的那条替代解释)**:16 个"取值从未被测"占不可判定的 76%,
若它们系统性偏向"footprint 改得最狠"的改写(父代 tile 在子代空间里跑不起来),
则 24% 会被**高估**。实测 `scripts/probes/undecidable_rewrite_bias.py`(3 项自检):
**16/16 全是 TPE 采样巧合,0 个是被拒**。
⇒ 该轴上无偏。**注意轴的范围**:排除的是"被拒"这一条路径,不是所有可能的偏差来源。

### 顺带的第二个发现:被接受的改写里有 5 个源码贡献为负

13 个 moved 案例里 **5 个的固定 knob 延迟差是负的**(−9.49%、−10.50%、−11.93%、−14.34%、−26.07%)
...
1. **把匹配点复测变成常规产物**,而不是事后探针:改写空间发布时,把父代最优 `ParamSet`
   ...
   今天 17/33 个已关闭改写碰巧有匹配点,靠的是 TPE 偶然采到;
   入队 anchor 让它变成 **33/33**,代价是**一个 trial**,而且该 trial 本身是有用的采样。
...
**样本量**:17 个可判定改写,来自 6 个 run、两块 4090 与继承自 box2 的记录。
