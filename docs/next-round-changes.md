...
1. **扩大起点**:归因起点包含**被拒点自己的邻域**,不只是 θ\*。
   `probe_top_k` 已朝这个方向,但它取"最优的前 K 个点",**仍全在 θ\* 一侧** ——
   对一个差 10 个 knob 的被拒点无帮助。
...
其代价 `correctness.py:242-243` 已实测:**批量每个配置边际 7 ms、单独跑约 16.7 s**。
回退点互相独立 ⇒ **可以批量提交**。

| | 9609 次(最坏) | 2326 次(仅一级) |
|---|---|---|
| **批量**(7 ms) | **67 秒** | 16 秒 |
| 逐个(16.7 s) | 44.6 小时 | 10.8 小时 |

⇒ **批量提交是这个方案可行与否的全部,差 2400 倍。**
实现时必须把回退点作为**一批** screen 提交,并且**要有测试钉住这一点** ——
一个"退化为逐个提交"的回退路径会让功能变得不可负担却**不报错**。
...
**⚠ 我先前把这个方案估贵了三个数量级**:我写"`compile_s` 在本项目里达数十秒",
并据此判定这是"编译密集搜索"。实测 **1821 个 trial 的 `compile_s` 中位 0.42 s、p90 0.45 s、最大 1.53 s**。
数十秒和 ptxas 吃 111 GiB 那两个数字来自**某个 agent 自己写的病态 PTX**
([[agent-script-can-oom-the-whole-box]]),不是正常 trial 的编译 —— 拿它定价是引错了量。
...
4. **§4.5 归因射程(本轮新增,**代价已量完**)** —— **这是 C2 零投递的根因**,
   ...
   **代价不再是未知**:388 个被拒配置、贪心最坏 9609 次评估,走 compile-only screen
   **批量 67 秒**(逐个则 44.6 h)⇒ 可做,但实现必须批量提交且有测试钉住。
