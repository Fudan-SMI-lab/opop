...
2. **多 knob 联合消融**:把被拒点**向 θ\* 逐维回退**,求使其装得下的**最小维度集合**。
   ...
   **代价必须先测**:今天每道墙 0.49 s 是**单 knob** 的价钱;逐维回退是组合搜索,
   须先在离线记录上算出实际需要多少次评估,再决定是否值。
...
4. **§4.5 归因射程(本轮新增,零机时可先量代价)** —— **这是 C2 零投递的根因**,
   而且比上面第 3 项更靠前:第 3 项问"覆盖率能不能扩大",§4.5 说明**为什么当前几乎全部归因失败**
   (被拒点与 θ\* 差 3–10 个 knob,单 knob 消融装不下)。
   先在离线记录上算"逐维回退需要多少次评估",那一步零机时;
   代价可接受再实现,且必须带"仍有拒绝被判 not_attributed"的正对照,
   否则放宽退化成拆门。
