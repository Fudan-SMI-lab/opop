...
## 更正:我先前对 `rewriter-18766c25` 的报告是错的

我曾向用户报告"第一个族三次 rewriter 尝试烧掉 1.96h **零产出**",并用它对比第二个族的
109 秒成功调用。**查磁盘后这个说法不成立**:

```
sandboxes/rewriter-18766c25/rewrites/rw_1.py   7152 字节,写入时间 21:47:00
该 call 的事件时间线:
  19:52:43 AGENT_CALL_STARTED
  21:00:20 AGENT_CALL_FAILED  attempt 1   (4057s,那次内存跑飞的)
  21:25:20 AGENT_CALL_FAILED  attempt 2   (1500s 空闲超时)
  21:50:20 AGENT_CALL_FAILED  attempt 3   (1500s;但 rw_1.py 已在 21:47:00 落盘)
```

**产物在 attempt 3 被砍前 3 分 20 秒就已经写完了**,是因为"调用没返回"而被丢弃。
所以真实情况是"1.96h + 一份写完的改写被扔掉",不是零产出。

四个 rewriter call 的全貌:

| call | 产物 | 失败次数 | 结局 |
|---|---|---|---|
| rewriter-18766c25 | 7152 B @ 21:47:00 | 4 | **产物被丢弃** |
| rewriter-1558e0eb | 2627 B @ 21:51:36 | 0 | 收下(109 秒完成) |
| rewriter-9050fdd2 | 5337 B @ 22:20:09 | 0 | 收下 |
| rewriter-c04c4d9a | 5534 B @ 22:58:45 | 1 | attempt 2 收下 |

**这对结论的影响,分两面说清**:

- **加强**了"时间预算是错误工具"的论证:被时限砍掉的恰恰是**已完成的产出**,
  这已经是第三次观测到同一模式(前两次见
  [[opop-agent-self-verification-burns-the-timeout]])。
- **削弱**了我给用户的那个 109s vs 4057s 对比的干净程度:两者不是"有产出 vs 无产出",
  而是"产出被收下 vs 产出被丢弃"。差别在于**是否在超时前返回**,不在于是否做出了东西。

**由此得到一个新的、比时间预算更该做的改进(记录待议,不在本轮实施)**:
超时后**不要丢弃沙箱里已完成的产物**。`check_output` 本来就是从沙箱读文件的,
`invoke()` 在 transport 失败后可以先检查产物是否已存在且通过校验,再决定是否重试 ——
