...
- **本轮 S7(斜率进采样器)结果**:29–31 次触发、**0 次投递**。
  根因已查清且**不是 S7 本身**:被拒点与 θ\* 相差 3–10 个 knob,而归因做的是从 θ\* 出发的单 knob 消融
  ⇒ 装不下 ⇒ `not_attributed`(详见 `docs/next-round-changes.md` §4.5)。
  **正文应把这条写成"机制的适用条件"而不是"机制失效"**,并给出可修的方向。
