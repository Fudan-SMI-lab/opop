...
| — | **C1 资源维度对齐 + 到达物理上限** | **新增为第一条 contribution**。证据最强:两次配对同向(**−8.68% 同箱干净**、**−12.0% 跨箱不干净但混淆对我方不利**);三臂赢家 `n_binding` 3/2/1 且无 unreachable ceiling。**自变量是"资源向量 vs 资源标签",不是"有无对齐"**。**但"在多个已达上限者中取最优"已被两条实测否证**,不得写入 |
...
### 3.0 全节通用的四条硬约束(写进 Experimental Setup)

1. **报告 `final_reeval_ms`,不报调参过程中的 best**。
   再评测差异实测 ±2–4% 且**符号不固定**([[reeval-gap-is-the-real-number]])。
   **⚠ 已有结果里 L3:21 的 6.92 ms 未独立复测** ⇒ 它不满足本条,不能直接进主表。
2. **每个加速比必须标注"对哪个基线、是否同精度"。** 这三条口径不同,并列会误导:
   - L3:48 的 9.29x 是**同精度**胜 `torch_compile_tf32`;
   - L3:43 的 3.647x 胜同一基线但**候选 fp16、基线 tf32**,**不是同精度** ——
     诚实说法是"在过双精度 relaxed 门 + fp64 相对检查的前提下,比最强 torch 基线快 3.65 倍";
   - 同一任务换基线数字差很多(L3:43:3.647x vs `torch_compile_tf32`、4.589x vs `torch_compile`、
     6.055x vs `eager_tf32`、7.121x vs `eager`)⇒ **表头必须写死基线**,否则可以靠换基线"提升"结果。
3. **一切延迟比较用 median,不用 mean**。median 排名正确率 93.2% vs mean 64.8%;
   `LatencyStats.robust_ms` 是 median-else-mean。**mean 会命名尾部**(13.5 min vs 中位 1.1 min)。
4. **臂间对等必须按"总搜索量"而不是"每空间预算"对齐**。K 扩展会让一个臂多领整份预算:
...
**报什么**
- 主指标:**相对最强 torch 基线的 speedup**(`torch_compile_tf32`,median-based、`final_reeval`),
  按 level 分别给几何平均。**不要用 eager 作主基线** —— 同一任务上 eager 会把数字从 3.647x 抬到
  7.121x(见 §3.0 第 2 条),用最弱基线报最大数字是最容易被质疑的做法。
  可在附录同时给出 vs `eager` / `eager_tf32` / `torch_compile` 四列,让读者自己换算。
- 外加**成功率**(通过正确性门的任务比例)—— 这一项不能省,因为方法间失败率差异会让平均值失真。
- **精度口径必须逐任务标注**(§3.0 第 2 条):候选与基线是否同精度。
- **⚠ 不要报"平均 latency"**(用户原话提到的选项之一)。不同任务的延迟量级跨几个数量级
  (本项目实测跨任务从 1.41 ms 到 6.92 ms,而 level1 的逐元素算子会低一个量级)⇒
  绝对延迟的算术平均会被**最慢的那个任务支配**,方法间差异会被它淹没。
  **正确做法是"每任务先归一化成 speedup,再取几何平均"** ——
  speedup 是比值,几何平均才是比值的正确聚合方式(算术平均会偏向大比值)。
...
#### (a) C1 消融:**资源向量 vs 资源标签**(不是"关闭对齐")

- **⚠ 自变量要写准**。已有的两次配对是 **vector 臂 vs label 臂**:
  两臂**都**拿到资源维度信息,区别是**向量形式(逐维度数值 + binding polarity)** 还是
  **标签形式**。**不是"有对齐 / 无对齐"** —— 照后者写会把消融的自变量描述错。
  机制在 `evaluation/dimensions.py` + `digest.py`;`for_prompt()` 是唯一允许产出 agent 可见
  资源文本的函数,入口第一行 `assert_no_raw_vector(d)`。
- 两次配对同向:

  | 配对 | vector 臂 | label 臂 | 差距 |
  |---|---|---|---|
  | S2(box2,**同箱、干净**) | 2.84 ms | 3.11 ms | **−8.68%**(噪声底的 3.7 倍) |
  | step 3(arm2 vs arm1,**跨箱、不干净**) | 2.49 ms | 2.83 ms | **−12.0%** |

- **第二次必须带限定**:跨箱不干净,但**混淆方向对 vector 臂不利** ——
  arm2 所在 box4 的 CPU 慢 23.8%(`compile_s` p50 0.423 s vs box1 0.321 s,+32%)、
  且少跑 40 个 trial,仍然赢 12%。**这是"混淆对我们不利"而非"已排除混淆",措辞不能省。**
- **"到达上限"是落盘量而非修辞**(`DIMENSION_STATE.binding`,三臂赢家):

  | 臂 | 赢家 binding 维度 | 数量 | unreachable ceiling |
  |---|---|---|---|
  | arm1 | occupancy, n_regs, n_spills | 3 | 无 |
  | arm2 | occupancy, n_regs | 2 | 无 |
  | arm3 | occupancy | 1 | 无 |

  三臂赢家都停在某个资源墙上,且无"上限测不到"的情况。
  **注意极性**:occupancy **低**才算绑定、`shared_bytes` **高**才算,极性须显式声明。
- **可直接复用已有 run,不需要新机时。**
...
| 用处 | 数据 | 来源 |
|---|---|---|
| C1 消融 | vector vs label 两次同向:**−8.68%**(同箱干净,噪声底 3.7 倍)、**−12.0%**(跨箱不干净,但混淆对 vector 不利);赢家 `n_binding` 3/2/1 且无 unreachable ceiling | `docs/contributions-current.md` §C1 |
...
| 三任务最终结果 | **L3:48** 1.41 ms(`final_reeval_ms`)/ **同精度** 9.29x 胜 `torch_compile_tf32`、94.7% 达实测 DRAM 屋顶 · **L3:43** 3.0126 ms(`final_reeval_ms`)/ 3.647x 胜 `torch_compile_tf32`,**⚠ 非同精度**(候选 fp16、基线 tf32)· **L3:21** 6.92 ms / 2.18x,**⚠ 未独立复测** | 三条 result memory |
