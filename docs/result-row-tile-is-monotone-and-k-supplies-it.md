...
| 16 | 29.8 | 29.9 | 43.0 |
| 32 | 25.0 | 29.1 | 29.4 |
| 64 | 18.6 | 28.5 | **23.5** ← best so far |
| 128 | **14.2** | **28.0** | *(1 trial, failed)* |
...
optimum is at its largest available value.
...
| `cand-3bf724d6` | `[16, 32, 64]` | **+128** → new best 28.6 → 28.0 |
| `cand-de802450` | `[16, 32, 64]` | **+128** → re-tune in flight |

Two of the three parameterizers stopped the row tile at 64. K noticed the boundary optimum in
both and added exactly the missing 128 — and on the one that has finished re-tuning, the new
value won (`tr-e14c7f45`, the incumbent with `QK_BLOCK_M` 64 → 128, everything else identical).
...
and K supplied the value the agent had left out. It is also a **repeated** finding rather than
a one-off, on two independent candidates in one run.
...
| `cand-de802450` | 2/8 = 25% | 1/1 = 100% (n=1) |
