...
**预期校准**:他们的增益与稀疏度相关,我们空间密度约 82%,应预期 +39% 的低端。
...
### 6. ECM 的合成规则 + ERM 的重叠系数 α —— 论价值应排第 2,论确定性是探索项

这一条来自较晚回来的第四路调研,详见 §3.1。放在这里是因为**它回答了设计文档里一个明确悬空的问题**:资源向量怎么合成一个数。

- **ECM**:`T = max(T_OL, T_nOL + T_data)` —— 不可重叠的核内周期与传输时间**相加**,其余与该和取 **max**。重叠假设是模型的**可证伪内容**,必须逐任务写下来并检验(ECM 自己就按层级报误差列)。
- **ERM**:`α = T_overlap / min(T_x,T_y) ∈ [0,1]`,在 `sum`(α=0)与 `max`(α=1)之间**连续插值,按资源对实测**。

**为什么值第 2**:α 直接决定我们一直在猜的那件事 —— **为省流量而多花的算力是免费的还是一比一付账的**。fp16/tf32、重算、融合三类改写全押在这个未知量上。
**为什么仍是探索项**:α 的定义需要"该维度单独的分量时间"。ECM/ERM 在 CPU 上从 OSACA 静态分析与分层传输模型取这些分量;**我们在 GPU 上取哪两个分量、怎么取,还没有可执行方案**(与§二缺口 1 是同一个瓶子)。子代理称"两点墙钟实验即可",我不采信这个乐观说法——它没说这两点具体是什么。**先写进设计,不排进关键路径。**

**同时借这一族的可用性事实**:Kerncraft(源码级自动建模)+ OSACA(汇编级 `T_nOL`/`T_OL` 切分)**整族 counter-free**,是唯一能在租用容器原样跑的先验工作族。我们已在读 cubin,这条路技术上是通的。
...
7. **把 LP 对偶/影子价格用于代码或 kernel 优化的瓶颈归因** —— 三种检索方式零命中,且检索方法有工作的正对照(§3.11)。**词汇与两条定理可借(互补松弛作自检、对偶之比作替代率),但不建 LP。**
