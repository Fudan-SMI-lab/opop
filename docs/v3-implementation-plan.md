...
| S2c 的前提 | 实测结论 |
|---|---|
| 付得起吗 | 一个资源点 614.6 ms(中位)/ 899 ms(均值)= **1/21~1/30 个计时 trial** |
| 字段拿得到吗 | `shared` 编译后 108/108;`n_regs`/`n_spills` **编译后 0/108,一次 launch 后 96/96**(launch 只要 0.4 ms) |
| 能不能用公式省掉 | **不能**:shared 线性拟合 **0/96** 精确命中(残差最大 121.4%);地图**不可分离**,1/8 的一步移动增量一致,同一移动的 shared 代价跨度 24576 B、寄存器跨度 127 |

**最后一行是 S2c 的必要性证明**:若有闭式公式或地图可分离,查表就是浪费。**两个否定结果一起把"实测局部地图"从选择变成唯一选项。**
